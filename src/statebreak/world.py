...
#: Entity fields that carry identity and can never be mutated through updates.
RESERVED_FIELDS = frozenset({"id", "type"})
...
def format_version(version_num: int) -> str:
    """Format a sequential version number as its public ``v<N>`` string."""
...
def _find_secret_path(value: Any, prefix: str = "") -> str | None:
    """Return the dotted path of the first value matching a secret pattern, if any."""
    ...
    if isinstance(value, dict):
        for k, v in value.items():
            ...
            found = _find_secret_path(v, child_prefix)
            if found is not None:
                return found
        return None
    if isinstance(value, (list, tuple)):
        for i, v in enumerate(value):
            found = _find_secret_path(v, f"{prefix}[{i}]")
            if found is not None:
                return found
        return None
    return None
...
class LocalWorld:
    ...
    def register_entity(self, entity_data: dict[str, Any]) -> None:
        ...
        # Reject credentials anywhere in the entity payload, at any depth.
        secret_path = _find_secret_path(cleaned)
        if secret_path is not None:
            raise ConfigurationError(
                f"security violation: entity {entity_id}.{secret_path} "
    ...
    def get_entity(self, entity_id: str) -> dict[str, Any] | None:
        ...
        ent["version"] = format_version(self._entity_versions[entity_id])
        return ent

    def get_entity_version(self, entity_id: str) -> str:
        if entity_id not in self._entities:
            raise UsageError(f"entity '{entity_id}' not found in world")
        return format_version(self._entity_versions[entity_id])
    ...
    def list_entities(self, entity_type: str | None = None) -> list[dict[str, Any]]:
        ...
        for eid in sorted(self._entities.keys()):
            ...
            if entity_type is None or ent.get("type") == entity_type:
                ...
                copy_ent["version"] = format_version(self._entity_versions[eid])
    ...
    def _apply_mutation(
        ...
        *,
        ...
        """Shared mutation path: validate, apply permitted fields, record the effect.

        ``allowed_fields`` restricts which update keys may be written (partial
        writes); ``None`` permits every field except the reserved identity keys.
        """
        if entity_id not in self._entities:
            return self._reject_mutation(
                ...
                before_version=DEFAULT_VERSION,
                after_version=DEFAULT_VERSION,
                kind=kind,
        ...
        cur_ver_str = format_version(cur_ver_num)

        # Idempotency check: if operation_id was already committed, replay its
        # original effect without applying anything again.
        ...
        if allowed_fields is None:
            candidates = [k for k in updates if k not in RESERVED_FIELDS]
        else:
            candidates = [
                k for k in allowed_fields if k in updates and k not in RESERVED_FIELDS
            ]
        ...
        eff = EffectRecord(
            effect_id=eff_id,
            operation_id=operation_id or f"op_{eff_id}",
            kind=kind,
            target=entity_id,
        ...
        return MutationResult(
            ...
            entity_id=entity_id,
            before_version=cur_ver_str,
            after_version=format_version(new_ver_num),
            effect=eff,
    ...
    def partial_update_entity(
        ...
        return self._apply_mutation(
            ...
            provider_id=provider_id,
            event_refs=event_refs,
            allowed_fields=tuple(applied_fields),
    ...
    def snapshot(self, captured_at: str | VirtualClock) -> StateSnapshot:
        ...
        for eid in sorted(self._entities.keys()):
            ...
            ent_clean["version"] = format_version(ver_num)
        ...
        aggregate_version = format_version(total_version_sum)
